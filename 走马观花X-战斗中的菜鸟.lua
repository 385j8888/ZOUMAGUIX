...
local window = library:new("走马观花X-战斗中的菜鸟")
...
gnn:Slider("跳高", "跳高设置", 50, 50, 100, false, function(value)
    lp.Character.Humanoid.JumpPower = value
end)

local gn = gn:section("主要功能",true)
gn:Toggle("透视敌人", "", false, function(state)
    bott = state  -- 同步阀门状态
    ...
    if state then
        --spawn(function()  -- 使用独立协程
            while bott do  -- 检测阀门状态
                  ...
                  local RunService = game:GetService("RunService")
                  local HIDDEN_UNITS = ReplicatedStorage:WaitForChild("HIDDEN_UNITS")

-- 创建文字容器
                  local tagFolder = Instance.new("Folder")
                  tagFolder.Name = "EnemyTags"
                  tagFolder.Parent = workspace

-- 追踪器存储表（使用弱引用防止内存泄漏）
                  local trackers = setmetatable({}, { __mode = "k" })

                  local function createTracker(model)
    -- 防重复机制：如果已经存在追踪器则跳过
    ...
    -- 创建追踪部件
                     ...
                     tracker.Size = Vector3.new(0.1, 0.1, 0.1)
                     tracker.Parent = tagFolder
    
    -- 创建3D文字
                     local billboard = Instance.new("BillboardGui")
                     billboard.Adornee = tracker
                     billboard.Size = UDim2.new(4, 0, 2, 0)
                     billboard.StudsOffset = Vector3.new(0, 2.5, 0)
                     billboard.AlwaysOnTop = true
                     billboard.Parent = tracker
    
                     local label = Instance.new("TextLabel")
                     label.Text = "敌人"
                     label.Size = UDim2.new(1, 0, 1, 0)
                     label.TextColor3 = Color3.new(1, 0.3, 0.3)
                      -- label.Font = Enum.Font.SourceHanSansBold
    ...
    -- 写入追踪系统
                     trackers[model] = {
                        basePart = basePart,
                        tracker = tracker,
                        lastValidPosition = basePart.Position
                        }
    
    -- 添加自动销毁监听
                     trackers[model].connection = model.AncestryChanged:Connect(function(_, parent)
                                 if not parent then
                                     tracker:Destroy()
                                     trackers[model] = nil
                                 end
...
-- 初始化现有模型（带防重复）
                  for _, model in ipairs(HIDDEN_UNITS:GetChildren()) do
                      if model:IsA("Model") and not trackers[model] then
                          createTracker(model)
                      end
                  end
...
-- 实时位置更新（带有效性验证）
                  RunService.Heartbeat:Connect(function()
                      for model, data in pairs(trackers) do
        -- 双重存在性验证
                          if model.Parent and data.basePart.Parent then
                              data.tracker.CFrame = data.basePart.CFrame 
                                  * CFrame.new(0, data.basePart.Size.Y/2 + 0.5, 0)
                              data.lastValidPosition = data.basePart.Position
                          else
            -- 自动清理无效条目
...
-- 监听新模型（带存在性检查）
                  HIDDEN_UNITS.ChildAdded:Connect(function(child)
                      if child:IsA("Model") and not trackers[child] then
                          createTracker(child)
                      end
                  end)
...
-- 跨服务器-客户端同步保障（可选）
    ...
    -- 将标签容器复制到所有客户端
                      tagFolder:SetAttribute("Replicated", true)
                  end      
                  wait(10)
            end
        --end)
...
gn:Toggle("自动跳过", "", false, function(state)
    jump = state  -- 同步阀门状态
    ...
    if state then
       --  pawn(function()  -- 使用独立协程
           while jump do  -- 检测阀门状态
                     wait(0.01)
                     game:GetService("ReplicatedStorage"):WaitForChild("Connection"):WaitForChild("inGame"):WaitForChild("Voting"):FireServer()
           end
    else
        print("关闭")
    end
