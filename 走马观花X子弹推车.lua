...
Button1 = "明白";
Duration = 15})
local window = library:new("走马观花X-子弹推车")
...
gm:Label("重要声明！此游戏是国产游戏，不要开太猛，支持国产，我不想把环境搞恶化！")
gm:Textbox("速度", "速度", "请输入速度", function(value)
    lp.Character.Humanoid.WalkSpeed = value
end)
gm:Textbox("跳跃", "跳跃", "请输入跳跃", function(value)
    lp.Character.Humanoid.JumpPower = value
end)
gm:Button("一键收纳",function()
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local RemoteEvent = ReplicatedStorage:WaitForChild("Msg"):WaitForChild("RemoteEvent")

-- 循环发送1到1000的所有数字（字符串形式）
for i = 1, 1000 do
    local args = {
        [1] = "\232\191\155\232\131\140\229\140\133",  -- 固定字段
        [2] = {
            [1] = tostring(i)  -- 将数字转为字符串，如"1", "2", ..., "1000"
        }
    }
...
gm:Toggle("自动收纳", "", false, function(state)
    jump = state  -- 同步阀门状态
...
gm:Toggle("自动丢物品", "", false, function(state)
    jumpp = state  -- 同步阀门状态
    
    if state then
       --  pawn(function()  -- 使用独立协程
           while jumpp do  -- 检测阀门状态
                          local player = game.Players.LocalPlayer
                          local character = player.Character or player.CharacterAdded:Wait()
                          local rootPart = character:WaitForChild("HumanoidRootPart")

                          local args = {
                              [1] = "\232\131\140\229\140\133\229\143\150\228\184\156\232\165\191",
                              [2] = rootPart.Position
                          }
...
gm:Button("直接通关",function()
workspace["\230\184\184\230\136\143\230\181\129\231\168\139"]["\230\184\184\230\136\143\231\187\147\230\157\159"].Value = true
end)