...
-- Chinese translate by XiaoAi, QQ:2586125757
-- ┻┳|                     |┳┻
--┳┻|  ∧            ∧   |┻┳
--┻┳|･ω･)           (•ω• |┳┻
--┳┻| ⊂ﾉ            \つ |┻┳
...
HeaderTextLabel.Text = "走马观花汉化海龟间谍"
...
CodeComment.Text = "--远程事件由TurtleSpy抓取，脚本由Interr#0421制作，脚本由小爱汉化"
...
DoNotStack.Text = "使用新参数激发时解除堆栈远程"
...
RemoteScrollFrame.ChildAdded:Connect(function(child)
    ...
    local connection = child.MouseButton1Click:Connect(function()
        ...
        if blocked then
            BlockRemote.Text = "停止屏蔽远程事件"
            BlockRemote.TextColor3 = Color3.fromRGB(251, 197, 49)
        else
            BlockRemote.Text = "屏蔽远程事件"
            BlockRemote.TextColor3 = Color3.fromRGB(250, 251, 255)
        end
        local iRemote = table.find(IgnoreList, lookingAt)
        if iRemote then
