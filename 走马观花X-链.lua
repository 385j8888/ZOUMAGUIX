...
local sd = workspace.GameStuff.PlayerStuff.Flashlight.Front.SurfaceLight
...
local runn = stats:WaitForChild("CanRun")
-- 示例：修改自身耐力值（需确保权限允许）
--stamina.Value = 100  -- 本地修改可能不会同步到服务器，需结合远程事件
local window = library:new("走马观花X-链")
...
gn:Toggle("无限奔跑耐力", "", false, function(state)
    naili = state  -- 同步阀门状态
...
gn:Toggle("无限理智值(不知道有啥用)", "", false, function(state)
    lizhi = state  -- 同步阀门状态
    local AI = workspace:FindFirstChild("Misc")
    if state then
       --  pawn(function()  -- 使用独立协程
...
gn:Toggle("无限战斗耐力", "", false, function(state)
    nailizd = state  -- 同步阀门状态
...
gn:Button("透视玩家",function()
    for i, v in next, game.Players:GetPlayers() do
        if v ~= lp then
            local esp = Instance.new("BillboardGui", v.Character)
            local esplabe = Instance.new("TextLabel", esp)
            esp.ResetOnSpawn = false;
            esp.AlwaysOnTop = true;
            esp.LightInfluence = 0;
            esp.Size = UDim2.new(1.75, 0, 1.75, 0);
            esplabe.Text = "";
            esplabe.Size = UDim2.new(0.0001, 0.00001, 0.0001, 0.00001);
            esplabe.BorderColor3 = Color3.fromRGB(255, 255, 255);
            esplabe.TextSize =10;
            esplabe.TextColor3 = Color3.fromRGB(255, 255, 255);
            esplabe.Text = "" .. tostring(v.Name)
        end
    end
end)
...
-- 添加红色白框高光
                     if not targetModel:FindFirstChildOfClass("Highlight") and bott==true then
                        local highlight = Instance.new("Highlight")
                        highlight.Name = "ModelHighlight"
                        highlight.FillColor = Color3.new(1, 0, 0)  -- 红色填充
                        highlight.OutlineColor = Color3.new(1, 1, 1) -- 白色边框
...
-- 添加3D文字标签
                     if not targetModel:FindFirstChild("TextLabel3D") and bott==true then
    -- 创建BillboardGui
    ...
    -- 创建文本标签
                         ...
                         textLabel.TextStrokeColor3 = Color3.new(1, 1, 1)  -- 白色描边
