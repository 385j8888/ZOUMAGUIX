...
gn:Textbox("攻击范围", "攻击范围", "请输入范围", function(value)
_hitboxsiz = Vector3.new(value, value, value)
end)
local siz = false
gn:Toggle("开启攻击范围", "", false, function(state)
    siz = state  -- 同步阀门状态
    ...
    if state then
       --  pawn(function()  -- 使用独立协程
           ...
           while siz do  -- 检测阀门状态
                 for _, box in pairs(workspace.Units:GetDescendants()) do
                    if box.Name=="HitboxSize" then
                        box.Value = _hitboxsiz
                    end
                 end
                 wait(10)
           end
...
local ats = false
gn:Toggle("开启攻速", "", false, function(state)
    ats = state  -- 同步阀门状态
...
local tsyxt = false
gn:Toggle("自动格挡(有点问题)", "", false, function(state)
    tsyxt = state  -- 同步阀门状态
...
local msss = false
gn:Toggle("自定义伤害(请先配置，伤害太大没用)", "", false, function(state)
    msss = state  -- 同步阀门状态
