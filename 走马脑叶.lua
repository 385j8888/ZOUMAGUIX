...
local tsyxt = false
gn:Toggle("透视所有异想体", "", false, function(state)
    tsyxt = state  -- 同步阀门状态
    ...
    if state then
      ...
      --   end)
             ...
             for _, objj in ipairs(workspace.Units:GetDescendants()) do
                     if objj:IsA("BillboardGui") then
                       objj:Destroy()  
                     end
             end
    end
...
gn:Toggle("锚定玩家(防大鸟魅惑)", "", false, function(state)
   -- hhk = state  -- 同步阀门状态
